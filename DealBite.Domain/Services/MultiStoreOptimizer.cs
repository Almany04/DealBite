...
namespace DealBite.Domain.Services
{
    public static class MultiStoreOptimizer
    {
        ...
        public static List<StoreComboResult> Optimize(List<ShoppingListItem> items,
                                                         List<ProductPrice> prices,
                                                         Dictionary<Guid, decimal> referencePrices,
                                                         List<Guid>? storeFilter=null,
                                                         int maxStoresPerCombo=3, int topN=10)
        {
            ...
            foreach (var combo in combinations)
            {
                ...
                {
                    var cheapest=prices
                        .Where(p => combo.Contains(p.StoreId) && p.ProductId == listItem.ProductId)
                        .OrderBy(p=>p.Price.Amount)
                        .FirstOrDefault();
                    if (cheapest == null)
                    {
                        var cheapestNull= new OptimizedItemResult 
                        {
                            IsAvailable = false,
                        ...
                        unavailableItems.Add(cheapestNull);
                    }
                    else
                    {
                        ...
                        if(referencePrices.TryGetValue(listItem.ProductId, out var refPrice))
                        {
                ...
                var comboResult = new StoreComboResult
                {
                    ...
                    ReferencePriceAmount = storeAssignments.Values.SelectMany(a => a.Items).Sum(i => (i.ReferencePriceAmount ?? i.UnitPrice) * i.Quantity)
